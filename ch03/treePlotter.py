# @description:使用文本注解绘制树节点
# @Author: 周健平
# @company: 山东大学
# @Time: 2020/10/02 21:06
...
def retrieve_tree(i):
    listoftrees = [{'no surfacing': {0: 'no', 1: {'flippers': {0: 'no', 1: 'yes'}}}},
                   ...
                   {'tearRate': {'reduced': 'no lenses', 'normal': {'astigmatic': {'yes': {'age': {'presbyopic': {'prescript': {'hyper': 'no lenses', 'myope': 'hard'}}, 'pre': {'prescript': {'hyper': 'no lenses', 'myope': 'hard'}}, 'young': 'hard'}}, 'no': {'prescript': {'hyper': 'soft', 'myope': {'age': {'presbyopic': 'no lenses', 'pre': 'soft', 'young': 'soft'}}}}}}}}]
...
#原因是firststr = mytree.keys()[0]字典取键值后不是直接生成列表，应该list强制转换成列表后再取其中的元素
#正确代码：firststr = list(mytree.keys())[0]
